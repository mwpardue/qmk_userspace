...
static uint32_t dynamic_macro_led1(uint32_t trigger_time, void *cb_arg) {
    static bool led_state = true;
    if (!is_launching) {
        led_state = !led_state;
        STATUS_LED_1(led_state);
    }
    return 100;
}

static uint32_t dynamic_macro_led2(uint32_t trigger_time, void *cb_arg) {
    static bool led_state = true;
    if (!is_launching) {
        led_state = !led_state;
...
bool dynamic_macro_record_start_kb(int8_t direction) {
    ...
    if (dynamic_macro_token == INVALID_DEFERRED_TOKEN) {
        if (direction == 1) {
            STATUS_LED_1(true);
            dynamic_macro_token = defer_exec(100, dynamic_macro_led1, NULL);
        } else if (direction == -1) {
            STATUS_LED_3(true);
            dynamic_macro_token = defer_exec(100, dynamic_macro_led2, NULL);
        }
    }
    return true;
}
